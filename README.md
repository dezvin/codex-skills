...
1. Если нужен полный preflight текущего Codex thread, использовать
   `preflight с экспортом` или сначала явно запустить `export-current-thread`.
2. Если файл экспорта создавать не нужно, использовать `preflight без экспорта`;
   тогда результат будет ограничен доступным контекстом.
...
лучше сразу указать: `с экспортом` или `без экспорта`. Без явного разрешения
skill сначала объяснит, зачем нужен локальный `.txt` экспорт, и спросит
согласие. Без экспорта он работает только по доступному контексту и помечает
результат как ограниченный.
